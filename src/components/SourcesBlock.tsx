...
const sourceLinkByTone: Record<NonNullable<SourceItem["tone"]>, string> = {
  default:
    "text-primary/85 underline underline-offset-4 hover:text-primary transition-colors",
  "very-light":
...
const sourceLinkGcNetwork =
  "text-muted-foreground/70 underline decoration-dotted decoration-muted-foreground/45 underline-offset-4 hover:text-muted-foreground/90 transition-colors";

const sourceSectionHeading =
  "text-xs uppercase tracking-[0.18em] font-medium text-primary/75";
    ...
    <li className="min-w-0 pl-6 -indent-6">
      {href ? (
        <>
          ...
          {item.note ? (
            <p className="mt-0.5 font-sans text-[10px] leading-[1.35] text-muted-foreground/55">
              {item.note}
            </p>
          ) : null}
        </>
      ) : (
        <span className="text-muted-foreground/80">{item.description}</span>
      )}
    </li>
  );
}
...
export function SourcesBlock({
  ...
  return (
    ...
    >
      <div className="rounded-lg border border-primary/25 bg-primary/[0.04] px-4 py-3.5 md:px-5 md:py-4">
        <button
          type="button"
          aria-expanded={open}
        ...
        >
          <ChevronDown
            ...
            aria-hidden
          />
          <span className="text-xs uppercase tracking-[0.18em] font-medium text-foreground/75 group-hover:text-foreground transition-colors">
            References
          </span>
        </button>
        {open ? (
          <div
            id={panelId}
            className="mt-3 border-t border-primary/15 pt-3 text-sm leading-[1.7] text-muted-foreground/80 space-y-4"
          >
            {sections.map((section, sectionIndex) => (
              <section
                key={section.label}
                className={cn(sectionIndex > 0 && "border-t border-primary/15 pt-4")}
              >
